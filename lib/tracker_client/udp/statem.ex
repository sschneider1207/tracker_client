defmodule TrackerClient.UDP.Statem do
  ...
  """
  @spec start_link(String.t, pid) :: :gen_statem.start_ret
  ...
  def init([url, parent]) do
    with uri           = URI.parse(url),
         {:ok, ipv4}   <- get_ipv4(uri),
         {:ok, socket} <- :gen_udp.open(0, [{:active, true}, :binary]),
         mon           = Process.monitor(parent),
         data          =  Data.new(uri: uri, socket: socket, ipv4: ipv4, mon: mon, parent: parent)
    do
      {:ok, :connection_request, data}
    else
      {:error, err} -> {:stop, err}
    end
  end
  ...
  def handle_event(:info, {:udp, socket, ipv4, port, <<3 :: 32-big-integer, tx :: 32-big-integer, msg :: binary>>}, _state, %{socket: socket, ipv4: ipv4, uri: %{port: port}, tx: tx} = data) do
    {:stop_and_reply, :normal, {:reply, data.from, {:error, msg}}}
  end
  ...
  def handle_event(:info, {:udp, socket, ipv4, port, response_packet}, :announce_response, %{socket: socket, ipv4: ipv4, uri: %{port: port}, tx: tx} = data) do
    ...
    reply = Announcement.new(interval, leechers, seeders, peers)
    {:stop_and_reply, :normal, {:reply, data.from, {:ok, reply}}}
  end
  def handle_event(:info, :retry, _state, %{request_num: 8} = data) do
    {:stop_and_reply, :timeout, {:reply, data.from, {:error, :timeout}}}
  end
  def handle_event(:info, :retry, state, data) do
    case retry_request(state, data) do
      {:ok, tx} ->
  ...
  def handle_event(:info, {:DOWN, mon, :process, parent, _reason}, _state, %{mon: mon, parent: parent}) do
    {:stop, :normal}
  end
  ...
  defp retry_request(:connection_response, data) do
    connect_request(data.socket, data.ipv4, data.uri.port)
  end
  defp retry_request(:announce_response, data) do
     announce_request(data.socket, data.ipv4, data.uri.port, data.conn_id, data.params)
  end
